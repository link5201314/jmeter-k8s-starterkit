...

{
    ...
    echo "  fi"
    echo "fi"
    echo "JOLOKIA_AGENT_JAR=\"/opt/jmeter/apache-jmeter/lib/jolokia-java-agent.jar\""
    echo "JOLOKIA_DEBUG=\"${JMETER_JOLOKIA_DEBUG:-false}\""
    echo "JOLOKIA_EXTRA=\"\""
    echo "if [ \"\${JOLOKIA_DEBUG}\" = \"true\" ]; then JOLOKIA_EXTRA=\",debug=true\"; fi"
    echo "JOLOKIA_JVM_ARG=\"-javaagent:\${JOLOKIA_AGENT_JAR}=host=0.0.0.0,port=8778,protocol=http\${JOLOKIA_EXTRA}\""
    echo "JVM_ARGS=\"${JMETER_SLAVE_JVM_HEAP_ARGS} \${JOLOKIA_JVM_ARG}\""
