...
if [ $rp_1 == 0 ]
   then
       echo "DROP PDB: ${pdb} restore point : ${rp_name} successfully !"
else
       echo "DROP PDB: ${pdb} restore point : ${rp_name} fail ! "
fi
...
set feedback off;
set heading off;
select name from v\\$restore_point where name='${rp_name}' con_id=(select con_id from v\\$pdbs where name='${pdb}' );
exit;
EOF
` 
...
if [ ${count_rp} == 0 ]
then 
    echo "No Restore Poinrt to delete"
    exit 1
fi
