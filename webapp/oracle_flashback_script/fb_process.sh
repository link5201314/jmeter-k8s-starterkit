...
where con_id=(select con_id from v\\$pdbs where name='${pdb}') and opname like '%Flashback%';
exit;
EOF`
...
if [ $rp_1 == 0 ]
then
    echo "No Restore process or restore complete"
    #echo "C"
    exit 1
fi


rp_2=`sqlplus -s "/ as sysdba" << EOF
set feedback off;
set heading off;
select round(sofar/totalwork *100,2) as complete from v\\$session_longops
where con_id=(select con_id from v\\$pdbs where name='${pdb}') and opname like '%Flashback%';
exit;
EOF`

echo "PDB : ${pdb} => Restore $rp_2 % complete"
#echo "R"
